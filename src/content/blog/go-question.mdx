...
The first change was in the scanner/lexer, whose job is to turn raw characters from your `.go` file and a stream of tokens.
...
multi-value assignment is always an `CallExpr`, because in normal Go, the only way to produce multiple values is a function call. So, for this workaround, we are treating the new `?` as though it was an inlined function
